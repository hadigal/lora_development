...
int main(void)
{
  ...
  else
  {
    ...
    logInfo("RESTING FROM DEEP SLEEP");
  ...
  while(true)
  {
    ...
    data.push_back('|');
    uint16_t toa = (uint16_t)(dot->getTimeOnAir(235));
    char toa_str[10];
    ...
    *********************************************************/
    ...
    //data.push_back((random_data >> 24) & 0xFF); // REALLOCATING THE 8 BIT VECTOR TO SUPPORT 32 DATA
    //data.push_back(random_data & 0xFF);
    ...
    while(itr < 60)
    {
        data.push_back((temp_data >> shift) & 0xFF); // REALLOCATING THE 8 BIT VECTOR TO SUPPORT 32 DATA
        data.push_back(temp_data & 0xFF);
        shift += 32;
        ++itr;
    }
    ...
    std::string random_data;
    std::string devEui = mts::Text::bin2hexString(dot->getDeviceId()).c_str();
    ...
    for(size_t itr = 0; itr < devEui.size(); ++itr)
    {
        data.push_back(devEui[itr]);
    }
    data.push_back('|');

    uint32_t plSize = random_data.size() + devEui.size() + 6;

    uint16_t toa = dot->getTimeOnAir(plSize);

    char strToa[10];
    sprintf(strToa,"%d",toa);

    for(size_t itr = 0; itr < strlen(strToa);++itr)
    {
        data.push_back(strToa[itr]);
    }

    //data.push_back((temp_data >> 239) & 0xFF); // REALLOCATING THE 8 BIT VECTOR TO SUPPORT 32 DATA
    //data.push_back(temp_data & 0xFF);
    ***************************************************************/
